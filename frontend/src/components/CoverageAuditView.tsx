      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 bg-[#161b22] border border-[#30363d] rounded-2xl space-y-2">
          ...
          <div className="text-2xl font-bold text-[#f0f6fc] tabular-nums">
            {isScanned ? '346 / 346 Files' : '— Files'}
          </div>
          <p className="text-xs text-[#8b949e]">
            {isScanned ? '100% of tracked repository files scanned at HEAD commit.' : 'Snapshot inspection pending initial scan run.'}
          </p>
        ...
        <div className="p-5 bg-[#161b22] border border-[#30363d] rounded-2xl space-y-2">
          ...
          <div className="text-2xl font-bold text-[#f0f6fc] tabular-nums">
            {isScanned ? '128 Commits' : '— Commits'}
          </div>
          <p className="text-xs text-[#8b949e]">
            {isScanned ? 'Full reachable commit history traversed back to root commit.' : 'Commit graph traversal pending scan trigger.'}
          </p>
          <div className="w-full bg-[#21262d] h-1.5 rounded-full overflow-hidden mt-3">
        ...
        <div className="p-5 bg-[#161b22] border border-[#30363d] rounded-2xl space-y-2">
          ...
          <p className="text-xs text-[#8b949e]">
            Ephemerally scanned in isolated worker, fully purged on complete.
          </p>
          <div className="w-full bg-[#21262d] h-1.5 rounded-full overflow-hidden mt-3">
      ...
      <div className="bg-[#161b22] border border-[#30363d] rounded-2xl overflow-hidden shadow-sm">
        <div className="p-5 border-b border-[#30363d] flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <span className="text-xs font-mono text-[#8b949e] bg-[#0d1117] px-3 py-1 rounded-lg border border-[#30363d] self-start sm:self-auto">
            {isScanned ? 'Run ID: scan-9f82c1a4' : 'Run ID: Pending Run'}
          </span>
        </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#30363d] text-[#c9d1d9] font-mono">
              <tr className="hover:bg-[#21262d]/50 transition-colors">
                ...
                <td className="px-5 py-3.5 text-[#8b949e]">HEAD Commit {isScanned ? '(346 files)' : '(Pending)'}</td>
                <td className="px-5 py-3.5 text-[#8b949e]">{isScanned ? '1.42s' : '—'}</td>
              ...
              <tr className="hover:bg-[#21262d]/50 transition-colors">
                ...
                <td className="px-5 py-3.5 text-[#8b949e]">128 Commits {isScanned ? '(Full Log)' : '(Pending)'}</td>
                <td className="px-5 py-3.5 text-[#8b949e]">{isScanned ? '2.89s' : '—'}</td>
                <td className="px-5 py-3.5">
                  {isScanned ? (
                    <span className="inline-flex items-center gap-1 text-[#3fb950] bg-[#238636]/15 px-2 py-0.5 rounded border border-[#238636]/30 text-[11px] font-sans font-medium">
                      <CheckCircle2 className="w-3 h-3" />
              ...
              <tr className="hover:bg-[#21262d]/50 transition-colors">
                ...
                <td className="px-5 py-3.5 text-[#8b949e]">{isScanned ? 'Findings Remediation' : 'Pending Findings'}</td>
                <td className="px-5 py-3.5 text-[#8b949e]">{isScanned ? '0.85s' : '—'}</td>
                <td className="px-5 py-3.5">
                  {isScanned ? (
