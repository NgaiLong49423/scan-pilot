import {
  ...
  FindingIssuePreviewDto,

  FindingIssueLinkDto,

  FindingRemediationPrPreviewDto,

  FindingRemediationPrLinkDto
 ...
 * Fetches secret-safe GitHub Remediation PR preview with masked diff and signed token.

 */

export async function fetchFindingRemediationPrPreview(findingId: string): Promise<ApiResult<FindingRemediationPrPreviewDto>> {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/v1/findings/${findingId}/remediation-pr-preview`, {
    ...
    if (response.ok) {
      ...
      const data: FindingRemediationPrPreviewDto = await response.json();
    ...
    if (response.status === 422) {
      ...
      return { status: 'ERROR', error: 'MANUAL_REMEDIATION_REQUIRED', statusCode: 422 };
 ...
 * Confirms and creates the GitHub Remediation Pull Request from preview token.

 */

export async function createFindingRemediationPr(

  findingId: string,

  payload: { previewToken: string }

): Promise<ApiResult<FindingRemediationPrLinkDto>> {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/v1/findings/${findingId}/remediation-pr`, {
    ...
    if (response.ok || response.status === 201) {
      ...
      const data: FindingRemediationPrLinkDto = await response.json();
    ...
    if (response.status === 409) {
      ...
      if (rawCode.includes('STALE_REVISION_ERROR')) {

        return { status: 'ERROR', error: 'STALE_REVISION_ERROR', statusCode: 409 };

      }

      if (rawCode.includes('PREVIEW_TOKEN_EXPIRED_OR_INVALID')) {

        return { status: 'ERROR', error: 'PREVIEW_TOKEN_EXPIRED_OR_INVALID', statusCode: 409 };

      }

      return { status: 'ERROR', error: 'REMEDIATION_PR_CONFLICT', statusCode: 409 };
    ...
    return { status: 'ERROR', error: 'REMEDIATION_PR_FAILED', statusCode: response.status };
...
export async function fetchFindingRemediationPrLink(findingId: string): Promise<ApiResult<FindingRemediationPrLinkDto>> {
  ...
  try {
    ...
    return { status: 'ERROR', error: 'LINK_UNAVAILABLE', statusCode: response.status };
