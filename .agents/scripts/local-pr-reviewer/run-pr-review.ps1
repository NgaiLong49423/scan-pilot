...
# Set and protect process working directory
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($apiKey)) {
        ...
        return @{ Status = "UNAVAILABLE"; ErrorCode = "MODEL_UNAVAILABLE"; ReviewedCount = 0 }
    }
    ...
    if (-not $preflight.Success) {
        ...
        return @{ Status = "UNAVAILABLE"; ErrorCode = $preflight.ErrorCode; ReviewedCount = 0 }
    }
    Write-Host " [OK]" -ForegroundColor Green

    # 2. Fetch Eligible Pull Requests with Pinned Repository
    Write-Host "Querying eligible same-repo PRs targeting [dev] on [$Repo]..."
    $queryResult = Get-EligibleDevPullRequests -Repo $Repo
    if (-not $queryResult.Success) {
        Write-Warning "Failed to query open pull requests for repository $Repo (Code: $($queryResult.ErrorCode))."
        return @{ Status = "UNAVAILABLE"; ErrorCode = $queryResult.ErrorCode; ReviewedCount = 0 }
    }

    $prs = $queryResult.PullRequests
    if ($PrNumber -gt 0) {
    ...
    foreach ($pr in $prs) {
        ...
        try {
            ...
            $remoteCheck = Check-RemotePrMarker -PrNumber $num -HeadSha $headSha -Repo $Repo
            ...
            $rawDiff = Get-PrDiffContent -PrNumber $num -Repo $Repo
            ...
            if ($DryRun) {
                ...
                $postResult = Publish-PrReviewComment -PrNumber $num -Body $commentBody -Repo $Repo
